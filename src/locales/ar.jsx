export const ar = {
  // Common
  Home: "الرئيسية",
  "Post Ad": "انشر إعلانك",
  Search: "بحث",
  Location: "الموقع",
  Language: "اللغة",
  "Loading...": "جاري التحميل...",
  Error: "خطأ",
  Submit: "إرسال",
  Cancel: "إلغاء",
  Next: "التالي",
  Previous: "السابق",
  Required: "مطلوب",
  Back: "رجوع",

  // Phone Listings
  "Mobile Phones": "الهواتف المحمولة",
  Page: "صفحة",
  of: "من",
  Total: "المجموع",
  "regular listings": "إعلان عادي",
  "Elite ads": "إعلانات النخبة",
  "Featured ads": "إعلانات مميزة",
  "All listings": "جميع الإعلانات",
  showing: "عرض",

  // Card
  Elite: "النخبة",
  Featured: "مميز",
  "Contact for price": "تواصل للسعر",
  Untitled: "بدون عنوان",
  Lebanon: "لبنان",
  recently: "مؤخراً",
  Chat: "دردشة",
  Call: "اتصال",
  WhatsApp: "واتساب",
  "Member since": "عضو منذ",
  "Verified Business": "عمل موثق",

  // Header
  Sell: "بيع",
  Login: "تسجيل الدخول",
  "All Categories": "جميع الفئات",

  // Home
  "OLX Lebanon": "أوليكس لبنان",
  "Find Cars, Mobile Phones and more...": "ابحث عن سيارات، هواتف نقالة والمزيد...",
  "All categories": "جميع الفئات",
  "Featured Ads": "إعلانات مميزة",
  "Recent Ads": "إعلانات حديثة",
  Apartments: "شقق",
  "View More": "عرض المزيد",

  // Post Ad
  "Post your ad": "انشر إعلانك",
  "Choose a category": "اختر الفئة",
  "Select a subcategory": "اختر الفئة الفرعية",
  Category: "الفئة",
  "Ad Details": "تفاصيل الإعلان",
  "Upload Images": "رفع الصور",
  Price: "السعر",
  "Ad Title": "عنوان الإعلان",
  "Enter ad title": "أدخل عنوان الإعلان",
  Description: "الوصف",
  "Enter ad description": "أدخل وصف الإعلان",
  "Select location": "اختر الموقع",
  "Ad posted successfully!": "تم نشر الإعلان بنجاح!",
  "Failed to post ad": "فشل نشر الإعلان",
  "Post Now": "انشر الآن",
  "Loading categories": "جاري تحميل الفئات",
  "Loading form fields": "جاري تحميل حقول النموذج",

  // Categories
  Vehicles: "المركبات",
  Properties: "العقارات",
  "Mobiles & Accessories": "الهواتف والإكسسوارات",
  Electronics: "الإلكترونيات",
  Furniture: "الأثاث",
  Business: "الأعمال",
  Pets: "الحيوانات",
  Kids: "الأطفال",
  Sports: "الرياضة",
  Hobbies: "الهوايات",
  Jobs: "الوظائف",
  Fashion: "الأزياء",
  Services: "الخدمات",
  Categories: "الفئات",

  // Footer - Popular Categories
  "Popular Categories": "الفئات الشائعة",
  Cars: "سيارات",
  "Flats for rent": "شقق للإيجار",
  "Apartments & Villas for Sale": "شقق وفيلات للبيع",

  // Footer - Trending Searches
  "Trending Searches": "عمليات البحث الرائجة",
  Bikes: "دراجات",
  Watches: "ساعات",
  Books: "كتب",
  Dogs: "كلاب",

  // Footer - About Us
  "About Us": "من نحن",
  "Contact Us": "اتصل بنا",
  "OLX for Businesses": "أوليكس للأعمال",

  // Footer - OLX
  OLX: "أوليكس",
  Help: "مساعدة",
  Sitemap: "خريطة الموقع",
  "Terms of use": "شروط الاستخدام",
  "Privacy Policy": "سياسة الخصوصية",

  // Footer - Follow Us
  "Follow Us": "تابعنا",

  // Footer - Copyright
  "Classifieds in Lebanon.": "إعلانات مبوبة في لبنان.",

  // Validation
  "This field is required": "هذا الحقل مطلوب",
  "Invalid price": "السعر غير صالح",

  // Additional
  "View more": "عرض المزيد",
  "Download OLX app now!": "حمّل تطبيق OLX الآن!",
  "Find amazing deals on the go.": "اعثر على عروض رائعة أثناء التنقل.",
  "Need help getting started?": "هل تحتاج مساعدة للبدء؟",
  "Review these resource to learn how to create a great ad and increase your selling chances":
    "راجع هذه الموارد لتتعلم كيفية إنشاء إعلان رائع وزيادة فرص البيع",
  "Tips for improving your ads and your chances of selling":
    "نصائح لتحسين إعلاناتك وزيادة فرص البيع",
  "All you need to know about Posting Ads": "كل ما تحتاج معرفته عن نشر الإعلانات",
  "You can always come back to change your ad": "يمكنك دائماً العودة لتعديل إعلانك",
  "Property Details": "تفاصيل العقار",
  "Additional Details": "تفاصيل إضافية",
  "Ad title": "عنوان الإعلان",
  "Mention the key features of your item (e.g. brand, model, age, type)":
    "اذكر المميزات الرئيسية للعنصر (مثل: العلامة التجارية، الموديل، العمر، النوع)",
  "Include condition, features and reason for selling":
    "قم بتضمين الحالة والمميزات وسبب البيع",
  "Select Location": "اختر الموقع",
  Name: "الاسم",
  "Enter your name": "أدخل اسمك",
  "Mobile Phone Number": "رقم الهاتف المحمول",
  "Contact Method": "طريقة التواصل",
  "OLX Chat": "دردشة OLX",
  Both: "كلاهما",
  "Phone Number": "رقم الهاتف",
  "Describe the item you're selling": "صف العنصر الذي تبيعه",
  "For the cover picture we recommend using the landscape mode":
    "للصورة الغلاف نوصي باستخدام الوضع الأفقي",
};